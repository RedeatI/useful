...
| Computer Use Probe V1 | `pnpm --filter @useful/cli test` | source 与真实解压 Agent Kit launcher 的 `useful.computer-use-probe.v1` 严格 shape/安装身份；`useful.computer-use.v1`、2 个 environment、9 项动作类型及 SHA-256 `a9bce07e51d533f830833d94ddc5fd53ae7f0b837da31edc8b68f64394a10cf7`；默认 provider disabled、`host-desktop` 拒绝、browser adapter 仅接口存在；capabilities/claims 明确无执行/provider/model/Action/MCP/GUI，并以 fetch、假 browser/input 命令及隔离 HOME/APPDATA/XDG 普通文件 manifest sentinel 检查本次命令未触发这些入口；后续 Agent probe 仍锁定 40 = 36 + 4 | JSON 文档认证、真实 browser/VM/OS 隔离、DNS/socket/redirect 强制、任意未布置入口、外部模型或 Agent 集成、真实用户输入、宿主配置外部审计、签名/来源/sidecar/发布授权、远程或平台执行 |
| Agent Connection Verification V1 | `pnpm --filter @useful/cli test` | 为固定 source/Agent Kit Useful MCP 入口在本进程生成连接候选、运行同次 probe、绑定 node/launcher 路径与安装身份，并锁定 40 = 36 + 4 及工具名 SHA-256；`claimScope/claims` 必须明确 self-reported、`documentAuthenticated:false`，V1 拒绝 `USEFUL_PROFILE`；Agent Kit 夹具须在解压后通过真实 launcher 至少验证一个 target，并以 sentinel 确认未执行输出 `commandArgv` | parser/Schema 对自报执行的认证，env/cwd 或 profile 绑定，Codex/Claude 或其他宿主已安装、已配置、会接受候选或已握手，宿主配置未读写的外部审计，签名、来源、sidecar、发布授权，launcher 无网络，外部宿主命令安全，远程发布 |
| Agent Connection Verification Set V1 | `pnpm --filter @useful/cli test` | `agent verify-all --launcher ... --json` 只运行一次 MCP self-probe，按 codex、claude-code、claude-desktop、mcp-servers-json 固定顺序生成四个 user-scope candidate-ready verification；四项 endpoint/probe 一致，均锁定 40 = 36 + 4 与工具名 SHA-256，任一失败则整体失败；source 与真实解压 Agent Kit 分别覆盖 `artifactVerified:false/true`（true 仅为 MANIFEST 自检）；PATH fake Codex/Claude/browser/input 与隔离 HOME/APPDATA/XDG sentinel 证明这些布置入口未执行且配置普通文件未改 | Schema/parser 或所有 true claims 对自报执行的认证；profile、target/scope/env/config/apply/install 覆盖；外部 Agent 已安装、已配置、已连接或会接受候选；任意未布置入口或 OS 级全局副作用审计；签名、来源、sidecar、发布授权、launcher 无网络、远程发布 |
| Settings Agent Connections Inspector | `pnpm --filter @useful/app test -- AgentConnectionPanel.spec.ts SettingsView.spec.ts`；`pnpm --filter @useful/protocol test` | 组件测试覆盖 mount/输入零 parse、零 invoke、零 clipboard 与零执行副作用，显式检查后的 valid/invalid/stale 状态、严格四个 user-scope/空 env 候选、XSS 字符串按文本渲染、仅显式复制及 clipboard 拒绝；协议测试覆盖 1 MiB、深度/节点/危险键/闭集与 cross-field fail-closed、Node parser + Ajv Schema 拒绝 parity，以及 browser bundle 无 Node/Ajv 依赖且 minified ESM ≤ 40 KiB | 真实 Tauri 桌面中 Settings 路由可达、WebView clipboard 权限/失败行为、原生视觉与键盘/读屏验收；也不证明 CLI 执行、IPC/子进程或宿主配置的 OS 级全局审计，不认证 self-reported JSON、外部 Agent 状态、签名/来源/发布授权，且不是 Release 证据 |
| Agent Kit 与 Agent 连接导出 | `pnpm --filter @useful/cli test` | 本次实际执行到的严格 5 bundle 闭集（新增 verification-set 仅进入现有 `useful.mjs`）、自包含导入、Agent/Computer Use protocol schema/provenance、Computer Use contract/browser-adapter provenance、逐包第三方许可证、清单/摘要，以及真实解压 `verify-all`、`computer-use probe`、`agent probe`、`agent plan`/只读 `doctor`/secret-free、current-host-only `agent export` 的无 monorepo 运行夹具 | 正式发布、远程 runner、签名、发布授权、任一外部 Agent 宿主配置、跨主机复用、宿主写入、真实 browser/input、联网、外部 Agent handshake 或真实隔离；Agent Kit 仍是 internal candidate |
