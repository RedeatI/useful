...
const PROTOCOL_AGENT_CONNECTION_VERIFICATION_SET_PROVENANCE_FILES = Object.freeze([
  "agent-connection-verification-set.mjs",
  "agent-connection-verification-set.d.ts",
]);
const PROTOCOL_COMPUTER_USE_PROBE_PROVENANCE_FILES = Object.freeze([
  "computer-use-probe.mjs",
  "computer-use-probe.d.ts",
]);
const COMPUTER_USE_CONTRACT_PROVENANCE_FILES = Object.freeze([
  "index.mjs",
  "index.d.ts",
]);
const COMPUTER_USE_BROWSER_ADAPTER_PROVENANCE_FILES = Object.freeze([
  "index.mjs",
  "index.d.ts",
]);
...
function workspaceResolver(repoRoot) {
  const packageSource = {
    ...
    "@useful/computer-use-browser-adapter": "packages/computer-use-browser-adapter/src/index.mjs",
    "@useful/computer-use-contract": "packages/computer-use-contract/src/index.mjs",
    ...
    "@useful/protocol/agent-connection-verification-set": "packages/protocol/src/agent-connection-verification-set.mjs",
    "@useful/protocol/computer-use-probe": "packages/protocol/src/computer-use-probe.mjs",
  ...
  try {
    result = await esbuild({
      ...
      nodePaths: [
        ...
        path.join(dependencyRoot, "packages/computer-use-browser-adapter/node_modules"),
        path.join(dependencyRoot, "packages/computer-use-contract/node_modules"),
...
async function addRuntimeResources(entries, repoRoot) {
  ...
  for (const relative of PROTOCOL_AGENT_CONNECTION_VERIFICATION_SET_PROVENANCE_FILES) {
    const source = `packages/protocol/src/${relative}`;
    addEntry(entries, `lib/provenance/protocol/${relative}`, normalizedText(await readSourceFile(repoRoot, source)));
  }
  for (const relative of PROTOCOL_COMPUTER_USE_PROBE_PROVENANCE_FILES) {
    const source = `packages/protocol/src/${relative}`;
    addEntry(entries, `lib/provenance/protocol/${relative}`, normalizedText(await readSourceFile(repoRoot, source)));
  }
  for (const relative of COMPUTER_USE_CONTRACT_PROVENANCE_FILES) {
    const source = `packages/computer-use-contract/src/${relative}`;
    addEntry(entries, `lib/provenance/computer-use-contract/${relative}`, normalizedText(await readSourceFile(repoRoot, source)));
  }
  for (const relative of COMPUTER_USE_BROWSER_ADAPTER_PROVENANCE_FILES) {
    const source = `packages/computer-use-browser-adapter/src/${relative}`;
    addEntry(entries, `lib/provenance/computer-use-browser-adapter/${relative}`, normalizedText(await readSourceFile(repoRoot, source)));
  }
