...
export function deterministicUuid(name, namespace = UUID_NAMESPACE) {
  const bytes = createHash("sha256").update(uuidBytes(namespace)).update(name, "utf8").digest().subarray(0, 16);
  // RFC 9562 UUIDv8: deterministic application-defined bytes without
  // presenting SHA-1 as a security-strength primitive.
