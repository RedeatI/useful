...
test("official Windows release uses the shared lock and exact Lite Full asset names", async () => {
  ...
  assert.match(release, /desktop-lite/);
  assert.match(release, /inputs\.scope == 'desktop-full'/);
  assert.match(release, /liteBuildAssets/);
  assert.match(release, /NOT-INCLUDED/);
  ...
  assert.match(release, /pnpm size:check --profile "\$env:SIZE_PROFILE" --json/);
  ...
  const checkIndex = release.indexOf('pnpm size:check --profile "$env:SIZE_PROFILE" --json');
