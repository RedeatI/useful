...
Useful 是一款给人直接使用、AI 也能轻松调用的桌面工具箱。它把常分散在浏览器标签页、命令片段和零散小程序里的任务集中到一处；用户可以直接使用其中的工具，AI 智能体也能通过 CLI、本地 MCP 服务和共享 Actions 调用同一套能力。
...
除非功能明确要求联网，工具输入留在本机。Useful 不包含 AI 模型。Useful 不修改 Codex、Claude 或其他 Agent 宿主的配置。

> [!IMPORTANT]
> Useful 是开发者预览版。官方**签名**安装包和生产更新源尚不可用。未签名的 Windows 桌面预览包已在
> [v0.1.0-beta.11](https://github.com/RedeatI/useful/releases/tag/v0.1.0-beta.11) 发布（推荐便携 zip）。
...
发布页：[v0.1.0-beta.11](https://github.com/RedeatI/useful/releases/tag/v0.1.0-beta.11)

**首选：便携 zip**

1. 下载 `Useful-0.1.0-beta.11-windows-x64-portable-lite.zip`
2. 解压
3. 打开内层文件夹 `Useful`
4. 运行 `Useful.exe`（保留旁边的 `portable.flag`）
5. 数据目录为 `Useful\data\`

可选：`Useful-0.1.0-beta.11-windows-x64-setup-lite.exe`。
可能出现 SmartScreen 警告。当前**不是** Authenticode 签名的生产包。

请同时下载发布页中的 `SHA256SUMS.txt`，校验便携包：

```powershell
$asset = "Useful-0.1.0-beta.11-windows-x64-portable-lite.zip"
